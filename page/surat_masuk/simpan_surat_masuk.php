...
switch (e($_GET['ref'])) {
	...
	//dd($_POST);
		if(isset($_POST['btnSimpan']) && e($_POST['btnSimpan']) === 'simpan_surat_masuk') {			
			...
			$sql = "insert into tp_arsip_surat_masuk (id_arsip_surat_masuk, nomor_urut,  nomor_berkas, nomor_surat_masuk, tanggal_masuk, pengirim, perihal, path_berkas, id_rekam) 
			values ('$last_id', '$nomor_urut', '$nomor_berkas', '$nomor_surat_masuk', '$tanggal_masuk', '$pengirim', '$perihal', '$path_berkas', '".getAuth()['username']."')";
			if(execStatementQuery($sql)) {
			// dd($tanggal_masuk);
